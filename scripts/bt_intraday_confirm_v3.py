...
def main():
    ...
    print('  In: %s~%s (%dd)' % (ins[0], ins[-1], len(ins)), flush=True)
    print('  Out: %s~%s (%dd)' % (outs[0], outs[-1], len(outs)), flush=True)
    print('  Computing features...', flush=True)
    rd, rb = st._load_data(ad[-1]); feat = st._compute_features(rd, rb, ad[-1])
    print('  Features: %d rows' % len(feat), flush=True)
    ...
    print('  Intraday: %d stocks' % len(idata), flush=True)
    ...
    for sn, dates in [('IN',ins),('OUT',outs)]:
        ...
        for mn,nb2,nv2,ui2 in modes:
            print('  Running %s %s...' % (sn, mn), flush=True)
    ...
    for sn in ['IN','OUT']:
        print('', flush=True)
        print('  == %s ==' % sn, flush=True)
        for mn,_,_,_ in modes:
            d = ar[sn][mn]
            print('  --- %s ---' % mn, flush=True)
            print('  Signals: %d  SkipB: %d  SkipV: %d' % (d['n'], d['sb'], d['sv']), flush=True)
            if d.get('iok',0)>0:
                print('  IntradayOK: %d  IntradayNO: %d  MissData: %d' % (d['iok'], d['ino'], d['imiss']), flush=True)
            for n in [1,2,3]:
                r = np.array(d['trades'][n]) if d['trades'][n] else np.array([])
                if len(r)==0: print('  T+%d: no data' % n, flush=True); continue
                ...
                print('  T+%d: n=%d WR=%.2f%% Avg=%.4f%% Med=%.4f%% PF=%.2f Payoff=%.2f' % (n, len(r), wr*100, avg*100, med*100, pf, po), flush=True)
    print('', flush=True)
    print('  == T+1 SUMMARY (OUT) ==', flush=True)
    ...
    for mn,_,_,_ in modes:
        ...
        print('  %25s  n=%4d  WR=%.2f%%(%+.2f%%)  Avg=%.4f%%(%+.4f%%)  PF=%.2f' % (mn, len(r), wr*100, (wr-bwr)*100, avg*100, (avg-bavg)*100, pf), flush=True)
    dd = ar['OUT']['D_intraday_confirm']
    if dd.get('etimes'):
        ts = dd['etimes']
        print('', flush=True)
        print('  Intraday entry time dist (OUT, %d trades):' % len(ts), flush=True)
        hc = {}
        for t in ts: h = t[11:13] if len(t)>13 else '?'; hc[h] = hc.get(h,0)+1
        for h in sorted(hc.keys()): print('    %s:xx  %3d' % (h, hc[h]), flush=True)
    print('', flush=True)
