...
OUT_COMPARE = BASE / "true_breakout_recall_v2_compare.csv"
OUT_MISS = BASE / "true_breakout_recall_v2_miss_reason.csv"
OUT_REVIEW = BASE / "true_breakout_recall_v2_review.md"
...
def write_review(cmp: pd.DataFrame, miss: pd.DataFrame) -> None:
    ...
    old = full[full["scenario"] == "mainline_s3_er1_er2"].iloc[0]
    new = full[full["scenario"] == "mainline_s3_er1_er2_er3"].iloc[0]
...
def main() -> None:
    ...
    q_score_60 = float(score.quantile(0.60))
    q_plat_55 = float(plat.quantile(0.55))
    q_comp_55 = float(comp.quantile(0.55))
    ...
    # S3
    q_chip_45 = float(chip.quantile(0.45))
    q_ind_50 = float(ind.quantile(0.50))
    q_score_55 = float(score.quantile(0.55))
    ech2_s3_base = (
        d["in_core_pool"].eq(0)
        & (trend >= 0.005)
        & (rs20 >= 0.56)
    ...
    for scope, sm in scope_masks.items():
        sdf = d[sm]
        rows.append(_metric(sdf, scope, "mainline_s3_er1_er2", m_old.loc[sdf.index]))
        rows.append(_metric(sdf, scope, "mainline_s3_er1_er2_er3", m_new.loc[sdf.index]))
    cmp_df = pd.DataFrame(rows)
    cmp_df.to_csv(OUT_COMPARE, index=False, encoding="utf-8-sig")

    miss_df = miss_reason(d, m_new)
