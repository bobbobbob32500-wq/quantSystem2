# -*- coding: utf-8 -*-
"""
聚宽研究环境 — 宽进突破策略（对齐本地 quantSystem 预设 wide_pool_strict_entry_v2）

选股层（对应 selection_relaxed_v1）：
  rs_quantile_min=0.78, min_signal_score=58, top_k=28,
  atr_quantile_max=0.55, box_max_range=0.095

买点层（对应 buy_tuning_v1）：
  breakout_buffer=0.004, breakout_max_chase=0.005,
  volume_confirm_ratio=1.42, volume_normal_ratio=1.08, max_intraday_gain=0.035

卖点（简化对齐本地 BreakoutStrategy.evaluate_position_exit）：
  持仓天数 >= max_hold_days(4) 或 最低价触及止损 或 收盘跌破 MA5 或 收盘跌破前一日最低价
  注意：A 股 T+1，建仓当日 manage_sell 不执行卖出（避免「可平仓数量不足」）。

使用说明（聚宽上跑回测）：
  1. 打开 https://www.joinquant.com → 研究 → 我的策略 → 新建策略 → 选择「股票策略」。
  2. 删除模板代码，将本文件全文粘贴进去；首行必须为：from jqdata import *
     （若从本仓库复制，请把下面 try/except 改成一行 from jqdata import *）
  3. 保存后点「回测」，设置起止日期、初始资金（如 100 万）；频率优先选「每天」。
     若用「分钟」回测，run_daily 仍会按时刻执行，但日志量更大；成交为零时请看日志里 [选股] 剔除统计与 [突破下单] 汇总。
  4. 若报 money 字段错误，可删掉 get_price 的 fields 里 money，仅用 volume*收盘价 近似成交额。

本地仓库路径：scripts/joinquant_wide_breakout_v2.py
"""

try:
    from jqdata import *  # noqa: F401,F403
except ImportError:
    # 仅本地 IDE 无 jqdata 时使用；聚宽环境请改为：from jqdata import *
    pass

# 聚宽回测日志；本地无 jqdata 时退化为 print，便于静态检查
try:
...
# ---------- 与本地 wide_pool_strict_entry_v2 一致的买点与选股阈值 ----------
P = {
...
def initialize(context):
    ...
    g.watchlist = {}
    # 记录买入：证券 -> {entry_idx, stop, ...}，entry_idx 为建仓日序号（用交易日计数简化）
    g.entry_meta = {}
    g.trade_day_idx = 0

    _jq_log.info(
        "策略初始化：基准=沪深300，选股阈值 min_score=%.1f rs_q_min=%.2f top_k=%d",
        P["min_score"],
        P["rs_q_min"],
        P["top_k"],
    )
...
def _box_pivot(box_days, high, low, close):
    ...
    # 聚宽旧版 pandas 无 pd.NA，用 np.nan 避免除零即可
...
def prepare_watchlist(context):
    ...
    if last_day is None:
        _jq_log.warn("[选股] previous_date 为空，跳过本日选股（回测首日常见）")
        return

    stocks = list(get_index_stocks("000300.XSHG"))
    if not stocks:
        _jq_log.warn("[选股] 沪深300成分股为空")
        return
    ...
    try:
        ...
        cd = None
        _jq_log.warn("[选股] get_current_data 失败，ST 过滤跳过: %s", e)

    # 统计各过滤环节剔除数量，便于排查「零成交」
    st = {
        "st": 0,
        "数据不足": 0,
        "成交额": 0,
    ...
    _jq_log.info(
        "[选股] 截止日=%s 沪深300=%d只 初筛=%d ret分位阈值=%.4f(rs_q=%.2f) 分位后=%d 观察池=%d 剔除=%s 异常样例=%s",
        last_day,
        len(stocks),
        n0,
        thr,
...
def try_breakout_and_trade(context):
    if not g.watchlist:
        _jq_log.info("[突破下单] 观察池为空，跳过（请先查看当日[选股]日志）")
        return
...
def manage_sell(context):
    """对齐本地 evaluate_position_exit 的简化版：时间止损、价格止损、跌破 MA5、跌破前低。"""
    today = context.current_dt.date()
    t1_skip = []
    for sec, pos in list(context.portfolio.positions.items()):
        ...
        # A 股 T+1：建仓当日 hold_days==0，不可卖出（否则报「可平仓证券数量不足」）
        if hold_days <= 0:
            t1_skip.append(sec)
            continue
        ...
        try:
            ...
            if df is None or len(df) < 3:
                _jq_log.warn("[卖出] %s 日线不足，跳过风控", sec)
                continue
            close = df["close"].astype(float)
            low = df["low"].astype(float)
            latest = df.iloc[-1]
            ...
            if hold_days >= P["max_hold_days"]:
                ...
                _jq_log.info("[卖出] %s 时间止损 持仓日=%d 上限=%d", sec, hold_days, P["max_hold_days"])
                continue
            if stop_loss > 0 and l_ <= stop_loss:
                order_target_value(sec, 0)
                g.entry_meta.pop(sec, None)
                _jq_log.info("[卖出] %s 止损 最低=%.3f 线=%.3f", sec, l_, stop_loss)
                continue
            if ma5 > 0 and c < ma5:
                order_target_value(sec, 0)
                g.entry_meta.pop(sec, None)
                _jq_log.info("[卖出] %s 跌破MA5 收=%.3f MA5=%.3f", sec, c, ma5)
                continue
            if prev_low > 0 and c < prev_low:
                order_target_value(sec, 0)
                g.entry_meta.pop(sec, None)
                _jq_log.info("[卖出] %s 跌破前低 收=%.3f 前低=%.3f", sec, c, prev_low)
                continue
        except Exception as e:
    ...
    if t1_skip:
        ...
        if len(t1_skip) > 10:
            tail += "..."
        _jq_log.info("[卖出] T+1 当日建仓共 %d 只，本时段不卖出，次日再检：%s", len(t1_skip), tail)
