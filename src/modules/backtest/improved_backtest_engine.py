...
解决问题：
1. 时间对齐机制（信号t-1，执行t）
2. 状态机设计（WATCH/HOLDING/COOLDOWN）
3. 资金与交易优先级
4. T+1严格约束
5. 多维卖出策略
6. 数据窗口跨日连续
...
logger = get_logger("improved_backtest_engine")
...
class ImprovedBacktestEngine:
    ...
    def run(self, start_date: str, end_date: str, show_progress: bool = True) -> Dict:
        ...
        for i, rec_date in enumerate(all_dates):
            ...
            self._run_single_day_improved(trade_date, candidates, intraday_data)
    ...
    def _run_single_day_improved(self, trade_date: datetime, candidates: List[str], 
        ...
        for trade_time, group in grouped:
            ...
            for symbol in list(self.positions.keys()):
                if symbol in bars:
                    sell_signal = self._check_sell_signal_improved(symbol, bars[symbol], trade_time)
                    if sell_signal:
            ...
            for symbol in candidates:
                if symbol in bars and symbol not in self.positions:
                    ...
                    buy_signal = self._check_buy_signal_improved(symbol, bars[symbol], trade_time)
                    if buy_signal:
                        self.pending_buy_signals.append(buy_signal)
            
            self._execute_sell_signals(bars, trade_time, trade_date_str)
            
            self._execute_buy_signals(bars, trade_time, trade_date_str)
    ...
    def _check_buy_signal_improved(self, symbol: str, bar: Dict, 
    ...
    def _check_sell_signal_improved(self, symbol: str, bar: Dict, 
                                     current_time: datetime) -> Optional[Signal]:
        """改进版卖出信号检测"""
    ...
    def _execute_sell_signals(self, bars: Dict[str, Dict], trade_time: datetime, 
        ...
        for signal in self.pending_sell_signals:
            ...
            if not self._check_t_plus_1_strict(symbol, trade_date_str):
            ...
            self._execute_sell(symbol, bar, trade_time, signal.signal_type, signal.reason)
    ...
    def _execute_buy_signals(self, bars: Dict[str, Dict], trade_time: datetime, 
        ...
        for signal in sorted_signals:
            if len(self.positions) >= self.config.max_positions:
                logger.debug(f"已达最大持仓数 {self.config.max_positions}，跳过买入")
                break
            ...
            success = self._execute_buy(symbol, bar, trade_time, signal.signal_type, signal.score)
            
            if success:
                pass
    
    def _check_t_plus_1_strict(self, symbol: str, current_date: str) -> bool:
        """严格的T+1检查"""
        if symbol not in self.positions:
            return False
    ...
    def _execute_buy(self, symbol: str, bar: Dict, trade_time: datetime, 
        ...
        buy_price = bar['open'] * (1 + self.config.slippage_rate)
        ...
        if cost > self.cash:
            shares = int(self.cash / buy_price / 100 / (1 + self.config.commission_rate)) * 100
            if shares < 100:
                return False
            cost = shares * buy_price * (1 + self.config.commission_rate)
        ...
        logger.debug(f"买入: {symbol} @ {buy_price:.2f} x {shares}股 | 信号: {signal_type} ({signal_score:.2f})")
        
        return True
    
    def _execute_sell(self, symbol: str, bar: Dict, trade_time: datetime, 
                      exit_reason: str, detail_reason: str = "") -> bool:
        """执行卖出"""
        if symbol not in self.positions:
            return False
        ...
        sell_price = bar['open'] * (1 - self.config.slippage_rate)
    ...
    def _force_close_positions(self):
        ...
        for symbol, position in list(self.positions.items()):
            sell_price = position.highest_price * (1 - self.config.slippage_rate)
            ...
            logger.info(f"  强制平仓: {symbol} @ {position.highest_price:.2f} ({pnl_pct*100:+.2f}%)")
